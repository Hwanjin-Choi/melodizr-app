...
export const mixTracks = async (tracks: any[]): Promise<string> => {
  try {
    const validTracks = tracks.filter((t) => t.uri);
    if (validTracks.length === 0) throw new Error("No tracks to mix");

    // Output file
    const outputFileName = `Melodizr_Mix_${Date.now()}.m4a`;
    const outputPath = `${FileSystem.cacheDirectory}${outputFileName}`;

    // Construct FFmpeg Command
    // ffmpeg -i input1.wav -i input2.wav -filter_complex ... -c:a aac output.m4a
    ...
    validTracks.forEach((track, index) => {
      ...
      // Calculate offset in milliseconds (ensure integer)
      const delay = Math.round(track.offset || 0);

      // Apply delay if needed
      // Apply delay and volume
      // 1. Calculate Volume
      const volume = track.type === "melody" ? 1.0 : 0.75;
      ...
      if (delay > 0) {
        ...
        // even if 0 delay, we might need it for consistency or simply omit adelay
        chain += `adelay=0|0,`;
      }

      chain += `volume=${volume}[a${index}];`;

      filterComplex += chain;
      mixInputs += `[a${index}]`;
    });

    filterComplex += `${mixInputs}amix=inputs=${validTracks.length}:duration=longest[out]`;

    // Use AAC encoder for M4A container (widely supported and built-in)
    const command = `${inputs} -filter_complex "${filterComplex}" -map "[out]" -c:a aac -b:a 192k "${outputPath}"`;
    ...
    if (ReturnCode.isSuccess(returnCode)) {
      ...
      console.error(`[FFmpeg] Failed: ${failStackTrace}`);
